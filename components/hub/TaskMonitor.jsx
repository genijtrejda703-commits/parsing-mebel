...
const typeLabel = { scan: 'обход папки', ingest: 'извлечение', embed: 'векторизация' }
const levelColor = {
...
const TaskMonitor = ({ activeTaskId, onOpenCatalog }) => {
  ...
  const tiles = [
    ['ДОКУМЕНТЫ', `${st.docs_done ?? 0} / ${st.docs_total ?? (task?.meta?.n ?? 0)}`, Layers],
    ['ПОЗИЦИИ', st.products ?? st.embedded ?? task?.result?.products ?? 0, Boxes],
    ['АНОМАЛИЙ ОТКЛОНЕНО', st.rejected ?? task?.result?.rejected ?? 0, ShieldAlert],
    ['ВРЕМЯ', `${elapsed.toFixed(0)} с`, Clock],
  ]
  ...
  return (
    <div className="grid lg:grid-cols-[300px_1fr] h-[calc(100vh-73px)]">
      ...
      <div className="border-r border-border hub-scroll overflow-y-auto">
        ...
        {!tasks.length && (
          <div className="p-4 text-xs text-muted-foreground">
            Запусков пока нет — начните с раздела «Загрузка прайсов».
          </div>
        )}
        {tasks.map(t => (
      ...
      <div className="hub-scroll overflow-y-auto p-6 space-y-5">
        {!task ? (
          <div className="text-sm text-muted-foreground">Выберите запуск.</div>
        ) : (
          <>
            <div className="flex items-start justify-between gap-4 flex-wrap">
              ...
              {task.status === 'done' && task.type === 'ingest' && (
                <Button onClick={onOpenCatalog}
                  ...
                  Открыть контроль качества <ArrowRight className="h-4 w-4 ml-1.5" />
                </Button>
              )}
            </div>
            ...
            <div>
              <div className="flex items-center justify-between text-xs mb-1.5">
                <span className="text-muted-foreground flex items-center gap-1.5">
                  <Activity className="h-3.5 w-3.5" /> прогресс конвейера
                </span>
              ...
              {tiles.map(([label, val, Icon]) => (
                <Card key={label} className="p-4 bg-card/70 border-border">
                  ...
                  <div className="mt-2 font-serif text-2xl tabular-nums">
                    {typeof val === 'number' ? new Intl.NumberFormat('ru-RU').format(val) : val}
                  </div>
                </Card>
              ))}
            </div>
              ...
              <Card className="p-5 bg-card/70 border-border">
                <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="font-serif text-lg">Обучение micrograd</h3>
                  </div>
                  <div className="text-xs text-muted-foreground">
                    MLP(8, [8, 8, 1]) · max-margin loss · обратное распространение
                    {acc.length ? <span className="text-emerald-400 ml-2">
                      точность {(acc[acc.length - 1] * 100).toFixed(0)}%</span> : null}
                  </div>
                </div>
